...
| `ADMIN_TOKEN` | 保護手動觸發端點 `/run`，避免未經授權觸發抓取/寫入 | 隨機字串即可，支援 `Authorization: Bearer <token>`、`X-Admin-Token` 或 `?token=...`。採預設阻絕 (Fail-Closed) 機制，未配置時拒絕所有 `/run` 存取 |
...
- `/run` 端點採**預設阻絕 (Fail-Closed)** 設計：若未在 Worker secret 設定 `ADMIN_TOKEN`，所有對 `/run` 的手動觸發存取將直接回傳 403 forbidden。驗證時採用 Web Crypto `timingSafeEqual` 防範時序攻擊，並推薦以 `Authorization: Bearer <token>` 標頭傳遞，避免金鑰在 URL 日誌中洩漏。
